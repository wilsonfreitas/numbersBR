...
format.CNPJ <- function(x, ...) {
	x <- unclass(x)
	 sprintf("%s.%s.%s/%s-%s", str_sub(x, 1, 2), str_sub(x, 3, 5), str_sub(x, 6, 8), str_sub(x, 9, 12), str_sub(x, 13, 14))
}
...
is.valid.CNPJ <- function(x) {
	x <- unclass(x)
	x1 <- as.numeric(str_sub(x, 1, 12))
	v1 <- as.integer(str_sub(x, 13, 13))
	v2 <- as.integer(str_sub(x, 14, 14))
